...
from utils import split_dataset_into_two, bag_of_words, split_into_words
...
def cross_validation(input_data, output_data, model, preprocess_type):
    J_values = []
    Accuracy_values = []
    for i in range(k_fold):
        ...
        train_input, test_or_val_input = preprocess(train_input.hotel_review, test_or_val_input.hotel_review, preprocess_type, is_bayes)
...
def nested_cross_validation(input_data, output_data, model, preprocess_type):
    J_values = []
    Accuracy_values = []
    for i in range(k_fold):
        ...
        J, accuracy = cross_validation(train_input, train_output, model, preprocess_type)
        J_values.append(J)
        Accuracy_values.append(accuracy)
